...
#include "verilated.h"
#if VM_TRACE
#include "verilated_fst_c.h"      // only when built with --trace
#endif
...
// Built WITHOUT --trace (VM_TRACE==0): tracer type/args/dump all vanish -> no FST code,
// big speedup, no giant waveform.fst. Built WITH --trace: identical to before.
#if VM_TRACE
  #define MTRACE_PARAM   , VerilatedFstC *m_trace
  #define MTRACE_ARG     , m_trace
  #define MTRACE_DUMP(t) m_trace->dump(t)
#else
  #define MTRACE_PARAM
  #define MTRACE_ARG
  #define MTRACE_DUMP(t) ((void)0)
#endif

void runCycles(unsigned int ncycles, Vtestharness *dut MTRACE_PARAM){
  for(unsigned int i = 0; i < 2*ncycles; i++) {
    sim_time += CLK_PERIOD_ps/2;
    dut->clk_i ^= 1;
    dut->eval();
    MTRACE_DUMP(sim_time);
  }
}
...
int main (int argc, char * argv[])
{
...
#if VM_TRACE
  // Open FST trace (only when built with --trace)
  ...
  runCycles(20, dut MTRACE_ARG);
  dut->rst_ni               = 0;
  runCycles(40, dut MTRACE_ARG);
