...
- Areas are in **square micrometres (µm²)**. 1 mm² = 1 000 000 µm².
...
| L | full FPU total (µm²) | FPU registers (µm²) | **FMA alone (µm²)** | rest = FPU − FMA (µm²) | FMA as % of FPU |
|---:|---:|---:|---:|---:|---:|
| 0 | 12 455.50 | 1 513.01 | **5 789.01** | 6 666.49 | 46.5 % |
| 1 | 12 951.54 | 1 760.07 | **6 135.66** | 6 815.88 | 47.4 % |
| 2 | 13 285.10 | 2 007.13 | **6 469.23** | 6 815.87 | 48.7 % |
| 3 | 13 636.86 | 2 254.19 | **6 820.98** | 6 815.88 | 50.0 % |
| 4 | 13 985.43 | 2 501.25 | **7 169.55** | 6 815.88 | 51.3 % |
| 5 | 14 337.51 | 2 748.31 | **7 521.63** | 6 815.88 | 52.5 % |
...
| Policy | total (µm²) | combinational (µm²) | registers (µm²) | ≈ flip-flops |
|---|---:|---:|---:|---:|
| **P0 — CPU-strict + accelerator round-robin** (default) | **452.94** | 424.22 | 28.73 | ~5 |
| **P1 — full round-robin** (no CPU priority) | 465.07 | 430.60 | 34.47 | ~6 |
| **P2 — QoS weighted (deficit-credit, 4:2:1)** | **927.60** | 732.24 | 195.35 | ~34 |
...
| Part | area | notes |
|---|---:|---|
| **Accelerator logic (total)** | **4 460.82 µm²** | FSM + batch accumulators + counters + operand routing |
...
| | area (µm²) | as % of one FMA |
|---|---:|---:|
| **AVOIDED** — one dedicated FMA (`fpnew_fma_multi`) | 5 789.01 | 100 % |
| *(or)* AVOIDED — a full second FPU (`cv32e40px_fp_wrapper`) | 12 455.50 | 215 % |
| **ADDED** — the sharing arbiter (P0, CPU-strict) | **452.94** | **7.8 %** |
| **NET SAVING** = FMA − arbiter | **5 336.07** | 92 % |
...
| L | avoided FMA (µm²) | added arbiter P0 (µm²) | arbiter as % of FMA | net saving (µm²) |
|---:|---:|---:|---:|---:|
| 0 | 5 789.01 | 452.94 | 7.8 % | 5 336.07 |
| 3 | 6 820.98 | 452.94 | 6.6 % | 6 368.04 |
| 5 | 7 521.63 | 452.94 | 6.0 % | 7 068.69 |
...
| | area (µm²) |
|---|---:|
| **AVOIDED** — two dedicated FMAs (2 × 5 789.01) | 11 578.02 |
| **ADDED** — one 3-requestor arbiter (P0) | 452.94 |
| **NET SAVING** | 11 125.08 |
...
3. **"No second FPU" is quantified:** to share the CPU's FMA you add a **453 µm² arbiter (7.8 % of one FMA,
   3.6 % of the full FPU)** instead of duplicating a **5 789 µm² FMA** — a **5 336 µm² net saving** per
   coprocessor; **11 125 µm²** for the dual-accelerator (one arbiter vs. two FMAs).
...
TSMC 40 nm G, RVT, SS/0.81 V/125 °C, 100 MHz, `compile_ultra`. Areas in µm².

| Run | what | total area | combinational | registers |
|---|---|---:|---:|---:|
| cv32e40px_fp_wrapper_L0 | full FPU, L=0 | 12 455.50 | 10 942.50 | 1 513.01 |
| cv32e40px_fp_wrapper_L1 | full FPU, L=1 | 12 951.54 | 11 191.47 | 1 760.07 |
| cv32e40px_fp_wrapper_L2 | full FPU, L=2 | 13 285.10 | 11 277.97 | 2 007.13 |
| cv32e40px_fp_wrapper_L3 | full FPU, L=3 | 13 636.86 | 11 382.67 | 2 254.19 |
| cv32e40px_fp_wrapper_L4 | full FPU, L=4 | 13 985.43 | 11 484.18 | 2 501.25 |
| cv32e40px_fp_wrapper_L5 | full FPU, L=5 | 14 337.51 | 11 589.19 | 2 748.31 |
| fpnew_fma_multi (from hier) | FMA, L=0 | 5 789.01 | — | — |
| fpnew_fma_multi (from hier) | FMA, L=1 | 6 135.66 | — | — |
| fpnew_fma_multi (from hier) | FMA, L=2 | 6 469.23 | — | — |
| fpnew_fma_multi (from hier) | FMA, L=3 | 6 820.98 | — | — |
| fpnew_fma_multi (from hier) | FMA, L=4 | 7 169.55 | — | — |
| fpnew_fma_multi (from hier) | FMA, L=5 | 7 521.63 | — | — |
| dma_apu_arbiter_P0 | arbiter, CPU-strict | 452.94 | 424.22 | 28.73 |
| dma_apu_arbiter_P1 | arbiter, full round-robin | 465.07 | 430.60 | 34.47 |
| dma_apu_arbiter_P2 | arbiter, QoS 4:2:1 | 927.60 | 732.24 | 195.35 |
| dma_fp_dot_accel_is | coprocessor logic (buffer black-boxed) | 4 460.82 | 2 041.92 | 2 418.90 |
